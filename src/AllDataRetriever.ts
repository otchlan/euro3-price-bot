...
async function fetchLiquidity() {
  try {
    console.log(`Fetching liquidity from contract at address: ${CONTRACT_ADDRESS}...`);
    const liquidity = await contract.liquidity();
    console.log('Fetched liquidity:', liquidity.toString());
...
// Function to fetch token addresses
async function fetchTokenAddresses() {
  try {
    console.log(`Fetching token addresses from contract at address: ${CONTRACT_ADDRESS}...`);
    const token0 = await contract.token0();
    const token1 = await contract.token1();
    console.log('Fetched token addresses:', { token0, token1 });
...
async function retrieveData() {
  try {
    ...
    return { globalState, liquidity, tokens, price };
  } catch (error) {
